...
class IconGenerator {
    static func generateAppIcon() {
        ...
        let iconSetPath = "build/ClipFlow.app/Contents/Resources/AppIcon.appiconset/"
        ...
        for (size, filename) in sizes {
            ...
            saveIcon(icon, filename: filename, size: size, path: iconSetPath)
        }
        
        createContentsJSON(path: iconSetPath)
    ...
    private static func saveIcon(_ image: NSImage, filename: String, size: Int, path: String) {
    ...
    private static func createContentsJSON(path: String) {
        let contentsJSON = """
        {
          "images" : [
            {
        ...
        let url = URL(fileURLWithPath: path + "Contents.json")
